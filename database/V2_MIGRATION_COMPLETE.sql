-- ============================================
-- ALDA GEL V2 - KOMPLE VERİTABANI MİGRATION
-- Yeni Sipariş Akışı: new_order → getting_ready → ready → assigned → picking_up → on_the_way → delivered
-- ============================================

-- ADIM 1: Mevcut constraint'i kaldır
ALTER TABLE packages DROP CONSTRAINT IF EXISTS packages_status_check;

-- ADIM 2: Yeni constraint ekle (tüm durumları içeren)
ALTER TABLE packages ADD CONSTRAINT packages_status_check 
CHECK (status IN (
    'new_order',
    'getting_ready',  -- YENİ: Restoran hazırlıyor
    'ready',          -- YENİ: Sipariş hazır, kurye ataması bekleniyor
    'assigned',
    'picking_up',
    'on_the_way',
    'delivered',
...
CREATE INDEX IF NOT EXISTS idx_packages_restaurant_id ON packages(restaurant_id);

-- ADIM 5: Yorumlar ekle
...
-- ADIM 6: Eski 'waiting' ve 'pending' değerlerini 'new_order'a çevir
UPDATE packages 
SET status = 'new_order' 
WHERE status IN ('waiting', 'pending');

-- ADIM 7: Kontrol - Yeni durumları listele
SELECT 
...
-- ADIM 8: Test - Mevcut siparişlerin durumlarını kontrol et
SELECT 
    status,
    COUNT(*) as count
FROM packages
